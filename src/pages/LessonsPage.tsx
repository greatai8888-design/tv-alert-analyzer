...
const TAG_FILTERS = ['ALL', 'RSI', 'MACD', '突破', '財報', '均線'] as const

function lessonStatusBadge(lessonType: string) {
  const lower = lessonType.toLowerCase()
  if (lower.includes('success') || lower === 'win') {
    return <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-primary-light text-primary-dark border border-primary/20">SUCCESS</span>
  }
  if (lower.includes('fail') || lower === 'loss') {
    return <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-tertiary-light text-tertiary-dark border border-tertiary/20">FAILED</span>
  }
  return <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-neutral/10 text-on-surface-variant border border-border">EXPIRED</span>
}
...
export default function LessonsPage() {
  ...
  const { data: lessons = [], isLoading } = useLessons(activeTag === 'ALL' ? undefined : activeTag)
  ...
  return (
    <div className="max-w-4xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="serif-heading text-[32px] md:text-[38px] text-on-surface">交易課程</h1>
        <p className="mt-1 text-[13px] text-on-surface-variant">從你的追蹤紀錄中學習</p>
      </div>
      ...
      {!isLoading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
          ...
          <h2 className="serif-heading text-[22px] text-on-surface">尚無課程</h2>
          <p className="text-sm text-on-surface-variant max-w-xs">
            完成更多追蹤交易後，系統將自動為你生成個人化的交易課程。
          </p>
        </div>
      )}
    </div>
