...
import type { SimTrade } from '../hooks/useSimTrading'
...
function HoldingCard({ trade }: { trade: SimTrade }) {
...
export default function SimTradingPage() {
  ...
  const [tab, setTab] = useState<'holdings' | 'history' | 'rules'>('holdings')
          ...
          {openTrades.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {openTrades.map(trade => (
                <HoldingCard key={trade.id} trade={trade} />
              ))}
            </div>
          )}
        </>
